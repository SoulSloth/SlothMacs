...
* Pre-setup
- Download =dired-single.el= to ="~/.emacs.d/misc-elisp/"=

* Add Load Path
In order to load certain =.el= scripts, we add this path.
...
** SQL
Done with =sqls= which can be installed with: =go install github.com/sqls-server/sqls@latest= [fn:8]
#+begin_src emacs-lisp
...
** Protobuf
Protobuf is an specification language for creating encodings, APIs, and generating code for both.
#+begin_src emacs-lisp
...
Must be downloaded from: https://github.com/emacsattic/dired-single and placed in =~/emacs.d/misc-elisp=
#+begin_src emacs-lisp
...
* Footnotes
[fn:8]https://github.com/sqls-server/sqls 
